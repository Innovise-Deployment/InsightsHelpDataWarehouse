...
.. glossary::
   ...
   configuration database
      The database where Insights stores all relevant reporting data. It is set up during the installation process and can be hosted on various types of databases.

   copy console
      A command-line tool to copy templates, reports and dashboards from one Insights Configuration database to another.
   ...
   Fusion
      The Insights database query engine, which leverages asynchronous querying in tandem with in memory query tree and map reduce technologies. This enables fast cross database querying and multi-step calculations. See :ref:`Insights_Fusion`.
   ...
   offline mode
      Insights license mode where the license key is validated using a token.

   online mode
      Insights license mode where the license key is validated by connecting to a remote license server.
   ...
   system variable
      A named place-holder that will be replaced by actual values while being used. See :doc:`/ref/spec_insights_system_variables`.
